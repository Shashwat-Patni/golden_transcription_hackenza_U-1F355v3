...
if "asr_pipeline" not in st.session_state:
    with st.spinner("Loading Whisper Model... This may take a minute on first run."):
        asr, model, processor, device, torch_dtype = build_pipeline(MODEL_SNAPSHOT)
        st.session_state.asr_pipeline = asr
        st.session_state.model = model
        st.session_state.processor = processor
        st.session_state.device = device
        st.session_state.torch_dtype = torch_dtype
...
with col2:
    ...
    candidates_input = st.text_area("Enter candidate transcriptions (one per line)",
                                   placeholder="Candidate 1 text...\nCandidate 2 text...",
                                   height=200)

if st.button("Run Assessment") and audio_file and candidates_input:
    ...
    try:
        with st.spinner("Generating Ground Truth (Whisper)..."):
            ...
            # Transcription
            t0 = time.time()
            result = st.session_state.asr_pipeline(
                str(tmp_path),
        ...
        with st.spinner("Computing Metrics & Ranking..."):
            ...
            st.dataframe(df.style.highlight_max(axis=0, subset=['CQS', 'Semantic Sim', 'Completeness', 'Precision', 'Recall'], color='lightgreen')
...
elif not (audio_file and candidates_input):
    st.info("Please upload an audio file and provide at least one candidate transcription.")
